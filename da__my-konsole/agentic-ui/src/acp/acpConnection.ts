...
import { getAcpWebSocketUrl } from './runtimeConfig';

// TODO(agentic-ui): stubbed - was Electron-only. Upstream used
// gooseServeLeaseRegistry's exit-detection message to distinguish a crashed
// local `goosed` child process from an ordinary connection error. This build
// talks to an externally-managed goosed and never spawns one, so there is no
// such signal; isGooseServeExitedError() below always returns false.
const GOOSE_SERVE_EXITED_USER_MESSAGE = '__agentic_ui_no_local_goosed_process__';
...
async function openConnection(generation: number): Promise<AcpConnection> {
  // TODO(agentic-ui): stubbed - was Electron-only (window.electron.getAcpUrl()).
  // Replaced with a runtime /config.json fetch; see acp/runtimeConfig.ts.
  const wsUrl = await getAcpWebSocketUrl();
  if (!wsUrl) {
