...
EOF
  ...
  # my-watchdog-tui, NOT "$DASH monitor". The monitor moved out of
  # my-konsole-dash when the panel became its own binary, so this launcher had
  # been exec'ing a subcommand that no longer exists — `watchdog` printed
  # "unknown dashboard 'monitor'" and exited 2. It read as the panel being
  # stale rather than the launcher pointing at the wrong program.
  {
    echo '#!/usr/bin/env bash'
    echo "# watchdog (generated by build.sh install) — full watchdog control."
    echo "exec \"$HOME/.local/bin/my-watchdog-tui\" \"\$@\""
