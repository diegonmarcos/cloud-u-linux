...
class _BaseUserscriptRunner(QObject):
    ...
    def _run_process(self, cmd, *args, env=None, verbose=False,
        ...
        # my-browser-qute: expose every QUTE_* var under a MYBROWSER_* alias too.
        # Both are set so the bundled/third-party userscripts written against
        # upstream's QUTE_* contract keep working under the new brand.
        self._env.update({'MYBROWSER_' + k[len('QUTE_'):]: v
                          for k, v in self._env.items() if k.startswith('QUTE_')})
