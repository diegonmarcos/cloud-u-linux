...
# my-browser-qute: branded alias for the internal qute:// scheme. Both are
# registered and handled; mybrowser:// URLs are normalised to qute:// on the
# way in, so the ~100 internal 'qute://...' strings (help, pdfjs, settings,
# session files, userscripts) keep working untouched.
_MYBROWSER = QByteArray(b'mybrowser')
...
class QuteSchemeHandler(QWebEngineUrlSchemeHandler):
    ...
    def install(self, profile):
        ...
        profile.installUrlSchemeHandler(_MYBROWSER, self)
...
def init():
    ...
    """
    if QWebEngineUrlScheme is not None:
        for name in (_QUTE, _MYBROWSER):
            assert not QWebEngineUrlScheme.schemeByName(name).name()
