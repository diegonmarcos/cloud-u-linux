...
parse_config() {
    ...
    # Expand ~ and defaults
    WORKDIR=$(echo "$WORKDIR" | sed "s|^~|$HOME|")
    ...
    REPOS=$(grep -E '^\s*"[^_][^"]*":\s*"(git@|https://)' "$CONFIG_FILE" | \
            sed 's/.*"\([^"]*\)"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1:\2/')

    # Extract private repos separately for visibility detection
    REPOS_PRIVATE=$(sed -n '/"private_repos"/,/}/p' "$CONFIG_FILE" | \
            grep -E '^\s*"[^_][^"]*":\s*"(git@|https://)' | \
            sed 's/.*"\([^"]*\)"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1:\2/')
...
cmd_status() {
    ...
    # Header
    printf "${C_BOLD}%-20s  %-8s  %-10s  %-8s  %-8s  %-8s  %-4s${C_RESET}\n" \
        "Repository" "Cloned" "Auth" "To Pull" "Local" "Unpushed" "CI"
    printf "${C_DIM}────────────────────  ────────  ──────────  ────────  ────────  ────────  ────${C_RESET}\n"
    ...
    for repo_name in $(get_repo_names); do
        ...
        # Auth type: check actual remote URL if cloned, otherwise config URL
        if [ -d "$repo_dir/.git" ]; then
            _url=$(git -C "$repo_dir" remote get-url origin 2>/dev/null)
        else
            _url=$(get_repo_url "$repo_name")
        fi
        case "$_url" in
            git@*) _auth="SSH" ;;
            https://*) _auth="HTTPS" ;;
        ...
        # Public/Private from config section
        if echo "$REPOS_PRIVATE" | grep -q "^${repo_name}:"; then
            _vis="Priv"
        else
            _vis="Pub"
        fi
        ...
        repo_data="${repo_data}${sort_cloned}${sort_pull}${sort_local}|${repo_name}|${cloned}|${auth}|${unpulled}|${uncommitted}|${unpushed}|${actions}\n"
    done

    # Sort and display
    printf "%b" "$repo_data" | sort | while IFS='|' read -r sortkey repo_name cloned auth unpulled uncommitted unpushed actions; do
        ...
        # Auth column
        case "$auth" in
            SSH/Priv)  printf "${C_MAGENTA}%-10s${C_RESET}" "$auth" ;;
            SSH/Pub)   printf "${C_CYAN}%-10s${C_RESET}" "$auth" ;;
            HTTPS/Pub) printf "${C_GREEN}%-10s${C_RESET}" "$auth" ;;
            *)         printf "${C_DIM}%-10s${C_RESET}" "$auth" ;;
        esac
        printf "  "
...
cmd_sync() {
    ...
    for repo_name in $(get_repo_names); do
        [ ! -d "$WORKDIR/$repo_name/.git" ] && continue
...
tui_shell() {
    while true; do
        ...
        case "$choice" in
            # Actions
            s|S) printf '\033[2J\033[H'; cmd_sync "$MERGE_STRATEGY"; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            p|P) printf '\033[2J\033[H'; cmd_pull "$MERGE_STRATEGY"; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            c|C) printf '\033[2J\033[H'; cmd_commit_all; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            u|U) printf '\033[2J\033[H'; cmd_push; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            f|F) printf '\033[2J\033[H'; cmd_status_fetch; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            # Lists
            1) printf '\033[2J\033[H'; cmd_untracked; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            2) printf '\033[2J\033[H'; cmd_unstaged; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            3) printf '\033[2J\033[H'; cmd_ignored; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            # Setup
            m|M) toggle_merge_strategy; sleep 1 ;;
            w|W) printf '\033[2J\033[H'; edit_workdir; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            l|L) cmd_clone_menu ;;
            d|D) printf '\033[2J\033[H'; check_deps; printf "\n${C_DIM}Press Enter...${C_RESET}"; read -r _ ;;
            r|R) ;; # Refresh
