...
# outputs you ask for, so a machine that installs `my-watchdog-bin` has no
...
# rescue initramfs.
#
# hashes.json is written by ship-my-watchdog.yml and committed. It is the seam
...
let
  ...
  arch = pins.${system} or (throw "my-watchdog: no published asset for ${system}");
...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    runHook preInstall
    install -Dm755 ${get "watchdog-d"}   $out/bin/watchdog-d
    install -Dm755 ${get "watchdog-tui"} $out/bin/watchdog-tui
    runHook postInstall
  '';
