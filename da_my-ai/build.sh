...
EOF
  ...
  local wrapper="$bindir/my-ai-usage-daemon"
  {
    ...
    echo "# machines behave the same: try the tray GUI, fall back to headless."
    echo 'GUI="'"$bindir/$GUI"'"'
    echo 'CLI="'"$bindir/$BIN"'"'
    echo 'if [ -x "$GUI" ] && [ -n "${WAYLAND_DISPLAY:-}${DISPLAY:-}" ]; then'
    echo '  "$GUI" && exit 0'
    echo '  echo "[my-ai] tray build exited non-zero — falling back to headless" >&2'
    echo 'fi'
    echo 'exec "$CLI" usage --daemon --interval 30'
  } > "$wrapper"
  chmod +x "$wrapper"
