...
| `git-add-secret` | secrets | irreversible | PreToolUse:Bash | `(^|[;&|])\s*git\s+(-[Cc]\s+\S+\s+)?add\b[^;|&]*(\.(env|key|pem|age|p12|pfx)([[:space:];|&]|$)|(^|/)\.secrets([[:space:];|&]|$)|secrets\.ya?ml([[:space:];|&]|$))` | git add of secret-shaped path — public-repo exposure risk | sops-encrypt secrets.yaml first; raw key material lives only in ~/git/cloud-vault |
