...
wrap_render_handler! {
    ...
    impl RenderHandler {
        ...
        fn on_accelerated_paint(
            ...
            // Multi-browser (chrome + content): key the shared texture map by
            // which CEF browser this paint came from, instead of one global slot.
            ...
            TEXTURE.with_borrow_mut(|textures| {
                textures.insert(browser_id, bind_group);
            });
        }
        ...
        fn on_paint(
            ...
            // Store the texture under this browser's identifier - chrome and
            // content each paint independently and must not clobber each other.
            TEXTURE.with_borrow_mut(|textures| {
                textures.insert(browser_id, bind_group);
            });
        }
...
thread_local! {
    pub static TEXTURE: RefCell<HashMap<i32, wgpu::BindGroup>> = RefCell::new(HashMap::new());
}
