...
# app has already had once at a lower level: the units said `c3-watchdog-d`, the
# release assets said `c3-watchdog`, and a laptop ran a three-week-old daemon
...
let
  ...
  pkgFor = system:
    if cfg.source == "prebuilt" then self.packages.${system}.c3-watchdog-bin
    else if cfg.tray then self.packages.${system}.c3-watchdog-tray
    else self.packages.${system}.c3-watchdog;
...
in
{
  options.services.my-watchdog = {
    enable = lib.mkEnableOption "c3-watchdog — the machine sampler and its panel";

    package = lib.mkOption {
      type = lib.types.package;
      default = pkgFor pkgs.stdenv.hostPlatform.system;
      defaultText = lib.literalExpression "c3-watchdog, or c3-watchdog-tray when tray = true";
      description = "The build to install. Both binaries — c3-watchdog-d and c3-watchdog-tui — come from it.";
    };
    ...
    policy = lib.mkOption {
      ...
      description = ''
        ...
        ~/.config/c3-watchdog/watchdog-policy.json under home-manager and to
        /etc/c3-watchdog/watchdog-policy.json on NixOS — both are search paths
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [ {
    ...
    if isHM then {
      # c3-watchdog-d and c3-watchdog-tui, on PATH, from the generation — which is
      # what makes `c3-watchdog-tui` the CURRENT build rather than whatever a
      # fetch last dropped in ~/.local/bin.
      home.packages = [ cfg.package ];

      xdg.configFile = lib.mkIf (cfg.policy != null) {
        "c3-watchdog/watchdog-policy.json".source = cfg.policy;
      };

      systemd.user.services.my-watchdog = {
        Unit = {
          Description = "c3-watchdog — machine sampler";
          After = [ cfg.startWith ];
        };
        # The store path, not a name in ~/.local/bin. A generation that says
        # what it starts is a generation you can read the answer off.
        Service = service ("${cfg.package}/bin/c3-watchdog-d" + lib.optionalString (!cfg.tray) " --no-tray");
      ...
      # the Ubuntu fleet.
      security.wrappers.c3-watchdog-d = {
        owner = "root";
        group = "root";
        capabilities = "cap_sys_ptrace,cap_dac_read_search,cap_net_admin+ep";
        source = "${cfg.package}/bin/c3-watchdog-d";
      };
      ...
      systemd.services.my-watchdog = {
        ...
        serviceConfig = service "${config.security.wrapperDir}/c3-watchdog-d --no-tray" // {
          User = "root";
