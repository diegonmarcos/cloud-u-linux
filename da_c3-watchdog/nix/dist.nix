...
{ lib, runCommand, c3-watchdog-bin, policy, mkService }:
...
let
  ...
  unit = mkUnit {
    desc = "c3-watchdog — machine sampler";
    exec = "@BIN@/c3-watchdog-d --no-tray";
  ...
  # that is gone.
  unitUser = mkUnit {
    desc = "c3-watchdog — machine sampler and guarded kill mailbox";
    exec = "%h/.local/bin/c3-watchdog-d";
  ...
  unitUserHeadless = mkUnit {
    desc = "c3-watchdog — machine sampler (headless)";
    exec = "%h/.local/bin/c3-watchdog-d --no-tray";
    after = "network.target";
    wantedBy = "default.target";
  };
in
runCommand "c3-watchdog-dist"
  { inherit unit unitUser unitUserHeadless; passAsFile = [ "unit" "unitUser" "unitUserHeadless" ]; } ''
  d="$out/c3-watchdog"
  mkdir -p "$d"
  cp ${c3-watchdog-bin}/bin/c3-watchdog-d   "$d/c3-watchdog-d"
  cp ${c3-watchdog-bin}/bin/c3-watchdog-tui "$d/c3-watchdog-tui"
  cp ${policy}                            "$d/watchdog-policy.json"
  cp ${./c3-watchdog-install.sh}             "$d/c3-watchdog-install.sh"
  ...
  chmod +x "$d/c3-watchdog-d" "$d/c3-watchdog-tui" "$d/c3-watchdog-install.sh"
  ...
  ( cd "$d" && sha256sum c3-watchdog-d c3-watchdog-tui watchdog-policy.json my-watchdog.service > SHA256SUMS )

  tar -C "$out" -czf "$out/c3-watchdog-dist.tar.gz" c3-watchdog
''
