#!/usr/bin/env bash
# c3-watchdog — fetch the GHA-built binary. Never compiles locally: the Rust
# build is heavy and the freeze-guard on this laptop exists because of exactly
# that kind of job.
set -euo pipefail
C='\033[0;36m'; N='\033[0m'
say() { printf "${C}[c3-watchdog]${N} %s\n" "$*"; }
...
case "${1:-fetch}" in
  fetch)
    ...
    # ExecStart=%h/.local/bin/c3-watchdog-d and my-konsole's `watchdog` launcher
    # has always exec'd ~/.local/bin/c3-watchdog-tui, while the release artifacts
    # are c3-watchdog and c3-watchdog-tui — so fetch installed two files
    ...
    ln -sf "$DEST/$BIN" "$(dirname "$LINK")/c3-watchdog-d"
    ...
    if gh release download "$TAG" --repo "$REPO" --pattern "$BIN-tui" --output "$tui_tmp" --clobber 2>/dev/null; then
      ...
      ln -sf "$DEST/$BIN-tui" "$(dirname "$LINK")/c3-watchdog-tui"
      say "panel: $BIN-tui"
    else
      rm -f "$tui_tmp"
    ...
    dist_tmp="$(mktemp -d)"
    if gh release download "$TAG" --repo "$REPO" --pattern c3-watchdog-dist.tar.gz \
      ...
      for u in user-my-watchdog.service user-my-watchdog-headless.service; do
        [ -f "$dist_tmp/c3-watchdog/$u" ] && install -m644 "$dist_tmp/c3-watchdog/$u" "$DEST/$u"
      done
      say "units: $DEST/user-c3-watchdog{,-headless}.service"
    else
    ...
    # every machine.
    mkdir -p "$HOME/.config/c3-watchdog"
    install -m644 "$(dirname "$0")/data/watchdog-policy.json" \
      "$HOME/.config/c3-watchdog/watchdog-policy.json" 2>/dev/null || true
    say "Fetched → $DEST/$BIN (restart c3-watchdog to load it)"
    ;;
  install)
  ...
  deploy)
    ...
    for h in $hosts; do
      ...
      ssh -o BatchMode=yes "$h" bash -s <<EOF
        systemctl --user stop c3-watchdog 2>/dev/null || true
        ...
        systemctl --user start c3-watchdog 2>/dev/null || true
...
EOF
      ...
      ssh -o BatchMode=yes "$h" 'mkdir -p ~/.config/c3-watchdog' </dev/null
      scp -q "$(dirname "$0")/data/watchdog-policy.json" "$h:.config/c3-watchdog/watchdog-policy.json"
      rm -f "$tmp"
