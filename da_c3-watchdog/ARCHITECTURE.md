# c3-watchdog — what talks to what
...
```
                       ┌──────────────────────────────────────┐
   /proc /sys cgroupfs │  c3-watchdog          (the daemon)   │
   ────────────────────┤  src/watchdog.rs                     │
...
| what | where |
|---|---|
| daemon (sampler + mailbox) | `da_c3-watchdog/src/watchdog.rs` |
| panel (all UI, all keys) | `da_c3-watchdog/src/tui/monitor/` |
...
| report stylesheet / renderer | `da_c3-watchdog/web/src/report.{scss,ts}` → committed `dist/` |
| Android app | `cloud-u-android/ac_cloud-watchdog` |
| Android ssh bridge | `cloud-u-android/ab_cloud-libs-shared/libs/watchdog` |

## Cross-repo references

* **cloud-u-android** — `ac_cloud-watchdog` runs `c3-watchdog-tui tui --serve`
