{
  description = "c3-watchdog — a machine sampler and its panel, installable on its own";

  # WHY THIS FLAKE EXISTS
  # c3-watchdog used to reach a machine only through that machine's system
  ...
  outputs = { self, nixpkgs, flake-utils, cloud-apps }:
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" ] (system:
      let
        ...
        # all — c3-watchdog-tui declares required-features = ["tui"], so a default
        ...
        mkWatchdog = { tray }: pkgs.rustPlatform.buildRustPackage {
          pname = if tray then "c3-watchdog-tray" else "c3-watchdog";
          version = "0.1.0";
          src = ./.;
      ...
      in
      {
        ...
        packages.dist = pkgs.callPackage ./nix/dist.nix {
          inherit (self.packages.${system}) c3-watchdog-bin;
        ...
        packages.c3-watchdog-tray = mkWatchdog { tray = true; };

        # `nix run github:diegonmarcos/cloud-u-linux?dir=da_c3-watchdog` opens the
        ...
        apps.default = {
          type = "app";
          program = "${crate}/bin/c3-watchdog-tui";
        };
        apps.c3-watchdog = {
          type = "app";
          program = "${crate}/bin/c3-watchdog-d";
        };

        devShells.default = pkgs.mkShell {
          name = "c3-watchdog-dev";
    ...
    // {
      ...
      #   imports = [ inputs.c3-watchdog.homeManagerModules.default ];
      ...
      #
      #   inputs.c3-watchdog.url = "github:diegonmarcos/cloud-u-linux?dir=da_c3-watchdog";
      #   imports = [ inputs.c3-watchdog.nixosModules.default ];        # or homeManagerModules
      #   services.my-watchdog.enable = true;
      #
      # Named for its job, beside the one that does the same job without nix:
      # nix/c3-watchdog-install.nix installs by DESCRIBING, nix/c3-watchdog-install.sh
      ...
      # different units.
      homeManagerModules.default = import ./nix/c3-watchdog-install.nix {
        inherit self;
        inherit (cloud-apps.lib) mkService;
      };
      homeManagerModules.c3-watchdog = self.homeManagerModules.default;
      nixosModules.default = self.homeManagerModules.default;
      nixosModules.c3-watchdog = self.homeManagerModules.default;
    };
}
