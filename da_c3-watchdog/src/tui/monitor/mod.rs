...
pub(crate) enum Source {
    /// c3-watchdog on the box itself: everything, every two seconds, and
...
/// `c3-watchdog-d` everywhere, and `libmywatchdog.so` inside an APK, where
...
fn start_sampler() {
    ...
    // updated yet still has c3-watchdog beside the panel, and a panel that
    // refuses to start the sampler it can see is worse than one that starts a
    // slightly older sampler.
    for name in ["c3-watchdog-d", "c3-watchdog", "libmywatchdog.so"] {
...
pub(crate) fn envelope_json_for(alias: Option<&str>) -> Result<String, String> {
    ...
    // over the terminal's exec service nobody did, and "is c3-watchdog
...
pub(crate) fn export_headless() -> Result<String, String> {
    ...
    let s = read_json(&p);
    if no_sample(&s) {
        return Err(format!("no usable snapshot at {p} — is c3-watchdog running?"));
    }
...
impl Monitor {
    ...
    fn render_source(&self, f: &mut Frame, area: Rect) {
        ...
        let (dot, dc, why) = if self.snap.is_null() {
            ("●", Color::Rgb(240, 72, 72), "no snapshot — is c3-watchdog running?".to_string())
        } else if age > 30.0 {
...
impl Dashboard for Monitor {
    ...
    fn render(&mut self, f: &mut Frame, area: Rect) {
        ...
        if self.about {
            ...
            al.push(kv2("watchdog source", format!("{repo}/tree/main/da_c3-watchdog")));
            al.push(kv2("releases", format!("{repo}/releases")));
            al.push(kv2("this binary", format!("{repo}/releases/tag/my-konsole-latest")));
            al.push(kv2("watchdog binary", format!("{repo}/releases/tag/c3-watchdog-latest")));
            al.push(kv2(
                "policy source",
                format!("{repo}/blob/main/da_c3-watchdog/data/watchdog-policy.json"),
            ));
            al.push(kv2(
                "publisher",
                ...
                // file.
                "c3-watchdog — da_c3-watchdog, its own product".into(),
            ));
