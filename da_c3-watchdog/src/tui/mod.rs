// The panel: c3-watchdog's own terminal UI.
//
// WHY IT LIVES HERE NOW
// This was a dashboard inside my-konsole — one of seven subcommands of a
// terminal emulator's companion binary — and c3-watchdog was a module inside
...
pub fn run(args: Vec<String>) -> std::io::Result<()> {
    ...
    if cmd == "export" {
        match monitor::export_headless() {
            ...
            Err(e) => {
                eprintln!("c3-watchdog-tui export: {e}");
                std::process::exit(1);
            }
        }
        return Ok(());
    ...
    if cmd == "snapshot" {
        ...
        match monitor::envelope_json_for(alias) {
            ...
            Err(e) => {
                eprintln!("c3-watchdog-tui snapshot: {e}");
                std::process::exit(1);
            }
        }
        return Ok(());
    ...
    if cmd == "tui" {
        ...
        if args.iter().any(|a| a == "--serve") {
            if let Err(e) = monitor::serve(n(1, 200), n(2, 64)) {
                eprintln!("c3-watchdog-tui tui --serve: {e}");
                std::process::exit(1);
            }
            return Ok(());
        }
        match monitor::tui_at(n(1, 200), n(2, 64)) {
            ...
            Err(e) => {
                eprintln!("c3-watchdog-tui tui: {e}");
                std::process::exit(1);
            }
        }
        return Ok(());
