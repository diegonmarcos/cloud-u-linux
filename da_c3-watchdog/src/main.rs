// c3-watchdog — the machine sampler, as its own product.
//
...
fn main() {
    ...
    if flag("--help") || flag("-h") {
        println!(
            "usage: c3-watchdog [--no-tray] [--once]\n       \
             c3-watchdog reclaim [clean|full] [MiB]\n       \
             c3-watchdog optimize <what>\n\
             \n\
    ...
    if args.first().map(String::as_str) == Some("reclaim") {
        let mode = match args.get(1) {
            ...
            Some(a) => match watchdog::Reclaim::parse(a) {
                Some(m) => m,
                None => {
                    eprintln!("c3-watchdog reclaim: expected 'clean' or 'full', got {a:?}");
                    std::process::exit(2);
                }
            },
        };
    ...
    if args.first().map(String::as_str) == Some("optimize") {
        ...
        match watchdog::optimize(what) {
            ...
            None => {
                eprintln!(
                    "c3-watchdog optimize: expected one of {}, got {what:?}",
    ...
    match (Tray { v }).disable_dbus_name(true).spawn() {
        ...
        Err(e) => {
            ...
            eprintln!("[c3-watchdog] no systray ({e}); sampling anyway");
            loop {
                std::thread::park();
            }
        }
